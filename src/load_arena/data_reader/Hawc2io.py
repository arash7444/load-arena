...
class ReadHawc2(object):
    ...
    def __init__(self, FileName, ReadOnly=0):
        ...
        if FileName.lower().endswith(".sel") or os.path.isfile(FileName + ".sel"):
            self._ReadSelFile()
        elif FileName.lower().endswith(".dat") and os.path.isfile(
            os.path.splitext(FileName)[0] + ".sel"
        ):
            ...
            self.FileFormat = "FLEX"
            self._ReadSensorFile()
        elif FileName.lower().endswith(".hdf5") or os.path.isfile(
            self.FileName + ".hdf5"
        ):
    ...
    def ReadAll(self, ChVec=None):
        ...
        if self.FileFormat == "HAWC2_BINARY":
            ...
            raise Exception(
                "Unknown file format {} for hawc2 out file".format(self.FileFormat)
            )
